...
public sealed class StepWindSettings
{
    ...
    /// versions are stored under. A root normally gets its folder leaf ("Documents"); a root
    /// whose leaf is already taken (by a live root, a removed root, or dead history in the
    /// store) gets a deterministic "leaf~hash8" id instead — which is how two folders that are
    /// both named "Documents" can be protected side by side without merging histories. Entries
    /// are kept after a folder is un-protected so re-adding the same path re-attaches its old
    /// history.
    /// </summary>
